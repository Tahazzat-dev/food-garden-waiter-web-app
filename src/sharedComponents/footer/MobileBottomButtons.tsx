...
export default function MobileBottomButtons() {
    ...
    return (
        <div className='w-full dark:border-t dark:border-slate-400 md:hidden bg-black fixed py-2.5 z-[9999] bottom-0 left-0'>
            <Container className='flex justify-between'>
                <Link className='flex flex-col gap-1 items-center justify-between' href="/" >
                    <Image src="/images/shared/home-icon.svg" width={25} height={40} alt="Home icon" />
                    <span className='text-white font-semibold text-sm sm:text-base'>Home</span>
                </Link>
                <button className='flex flex-col gap-1 items-center justify-between' >
                    <Image src="/images/shared/special.svg" width={30} height={40} alt="Offer icon" />
                    <span className='text-white font-semibold text-sm sm:text-base'>Offer</span>
                </button>
                <button onClick={() => dispatch(SET_EXPAND('CART_SHEET'))} className='relative flex flex-col gap-1 items-center justify-between' >
                    <div className='relative'>
                        <ShoppingCart fill='white' className='text-white h-7 w-7 cursor-pointer' />
                        {
                ...
                <button className='flex flex-col gap-1 items-center justify-between' >
                    <Image src="/images/shared/whatsapp.svg" width={30} height={40} alt="Whatsapp icon" />
