...
export default function Tables({ selectedTable, setSelectedTable }: Props) {
    ...
    return (
        <div className="table-container w-full flex max-w-full overflow-x-auto gap-2.5 py-1.5">
            {
                tables.map(table => <button type="button" onClick={() => setSelectedTable(table.id)} className={cn("duration-200 rounded-md px-2 pt-1 pb-0.5", selectedTable === table.id ? "bg-primary text-clr-text-body" : table.isBooked ? "bg-secondary text-white" : "bg-slate-100 text-clr-text-body")} key={"table_" + table.id}>
